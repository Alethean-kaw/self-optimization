...
import { EMPTY_MATCHER_NOTE, HOOK_DETECT_COMMAND, HOOK_REMIND_COMMAND, LEARNINGS_FILES, MATCHING_PROMPT_NOTE } from "../lib/constants.js";
...
export async function runDoctor(options, context) {
    ...
    const results = [];
    results.push((await exists(path.join(context.packageLayout.packageRoot, "dist", "cli.js")))
    ...
    const hookCommandsPresent = hooksSetupContent.includes(HOOK_REMIND_COMMAND) &&
        hooksSetupContent.includes(HOOK_DETECT_COMMAND);
    results.push(hookCommandsPresent
    ...
    const matcherVerificationAligned = hooksSetupContent.includes(MATCHING_PROMPT_NOTE) &&
        hooksSetupContent.includes(EMPTY_MATCHER_NOTE);
    results.push(matcherVerificationAligned
    ...
    if (await exists(workspaceLearningsPath)) {
        ...
        results.push(`[warn] Promotion metadata checks skipped because the workspace learnings file is missing: ${workspaceLearningsPath}`);
    }
    return `${results.join("\n")}\n`;
}
function getPromotionWarnings(content) {
    const warnings = [];
