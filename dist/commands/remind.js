...
export async function runRemind(options, context) {
    const now = context.now?.() || new Date();
    const sessionKey = firstNonEmpty(options.sessionKey, context.env.CLAUDE_SESSION_ID, context.env.CODEX_SESSION_ID, context.env.OPENCLAW_SESSION_KEY, context.env.SESSION_ID);
    const workspace = await resolveWorkspace(options.workspace || context.cwd);
    const cooldownSeconds = parsePositiveInteger(firstNonEmpty(options.cooldownSeconds, context.env.SELF_OPTIMIZATION_REMINDER_COOLDOWN_SECONDS), 1800);
