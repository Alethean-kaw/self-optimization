...
export interface ExtractSkillOptions {
  ...
  dryRun?: boolean;
  outputDir?: string;
  sourceLearningFile?: string;
  fromLearningId?: string;
}

export interface ExtractSkillContext {
  cwd: string;
...
function resolveOutputRoot(outputDir: string | undefined, context: ExtractSkillContext): string {
  if (!outputDir) {
...
function resolveLearningSourcePath(options: ExtractSkillOptions, cwd: string): string {
...
async function loadLearningSeed(fromLearningId: string, sourceLearningFile: string) {
